...
%     nonAnchorPoint = [-6.86635944700461,-29.6783625730994];
    ...
    for i=2:numOfAnchors
        ...
        addedNoise = rand * d(i) - d(i)/4;
        d_noisy(i, :) = d(i, :) + addedNoise;
        u_d(i) = log(d_noisy(i));
        
        b(i-1, :) = (anchors(i, 1)^2 + anchors(i, 2)^2 - d_noisy(i)^2 + d_noisy(1)^2);
    end
    S = ones(numOfAnchors-1, numOfAnchors-1) * d(1)^4 + diag(d(2:numOfAnchors).^4);% * exp(4*u_d(1)) + diag(exp(4*u_d(2:numOfAnchors)));
    % S2 = ones(numOfAnchors-1, numOfAnchors-1) * d_noisy(1)^4 + diag(exp(d_noisy(2:numOfAnchors)));
%     S = S .* (exp(8 * sigma_d^2) - exp(4 * sigma_d^2));
